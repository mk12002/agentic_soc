...
def main() -> int:
    ...
    md = ["# Feature verification", "",
          f"Generated by `python scripts/verify_features.py{' --browser' if args.browser else ''}{' --engine' if args.engine else ''}` "
          ...
          f"| Tests executed for this report | {sum(1 for v in results.values() if v != 'skip')} passed or failed, "
          f"{sum(1 for v in results.values() if v == 'fail')} failed |"]
    for name, detail, ok in evals:
        md.append(f"| {name} | {'✅' if ok else '❌'} {detail} |")
    if browser:
        md.append(f"| Browser tour (real server + browser, every screen, 3 roles) | {'✅' if browser[0] else '❌'} {browser[1]} |")
    if engine:
